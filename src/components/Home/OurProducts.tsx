...
const OurProducts = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const transformedCategories: DisplayCategory[] = response.data.map(
        (category) => ({
          id: category.id,
          categoryTitle: category.categoryTitle,
          categoryImage: category.categoryImage,
          features: category.subCategories.map((subCat) => subCat.name),
        })
      );
          ...
          >
            ...
            >
              ...
              >
                ...
                {category.features.map((feature: string, index: number) => (
                  <a
                    href={`/${generateLink(category.categoryTitle)}`}
                    className="hover:text-blue-600 transition-colors"
                  >
